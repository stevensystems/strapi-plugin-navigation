...
const { type: itemType, additionalFields: configAdditionalFields } =
  require("../content-types/navigation-item").lifecycle;
...
module.exports = ({ strapi }) => {
  ...
  return {
    ...
    async getById(id) {
      ...
      const entity = await strapi.query(masterModel.uid).findOne({ where: { id } });

      const entityItems = await strapi.query(itemModel.uid).findMany({
        where: {
          master: id
        },
        limit: Number.MAX_SAFE_INTEGER,
    ...
    async config(viaSettingsPage = false) {
      ...
      if (additionalFields.includes(configAdditionalFields.AUDIENCE)) {
        const audienceItems = await strapi.query(audienceModel.uid).findMany({
          limit: Number.MAX_SAFE_INTEGER
        });
        extendedResult = {
          ...extendedResult,
    ...
    async configContentTypes(viaSettingsPage = false) {
      ...
      const eligibleContentTypes = await Promise.all(
        config.contentTypes
          .filter(
            (contentType) => !!strapi.contentTypes[contentType] && utilsFunctions.isContentTypeEligible(contentType)
          )
          .map(async (key) => {
            ...
            if (isSingleType) {
              ...
              return isAvailable === 1 ? returnType(true) : viaSettingsPage ? returnType(false) : undefined;
            }
      ...
      return eligibleContentTypes
        ...
        .map(({ key, available }) => {
          ...
          const findRouteConfig = find(get(strapi.api, `[${modelName}].config.routes`, []), (route) =>
            route.handler.includes(".find")
          );
          ...
          const labelSingular =
            name || upperFirst(relationNameParts.length > 1 ? relationNameParts.join(" ") : relationName);
          return {
            uid,
            name: relationName,
            isSingle,
    ...
    async getRelatedItems(entityItems) {
      ...
      const groupedItems = Array.from(relatedTypes)
      ...
      const data = new Map(
        (
          await Promise.all(
            Object.entries(groupedItems).map(async ([model, related]) => {
              const relationData = await strapi.query(model).findMany({
                where: {
                  id: { $in: map(related, "related_id") }
                },
                populate: config.contentTypesPopulate[model] || []
              });
              return relationData.flatMap((_) =>
                Object.assign(_, {
                  __contentType: model,
      ...
      return entityItems.map(({ related, ...item }) => {
    ...
    async post(payload, auditLog) {
      ...
      const existingEntity = await strapi.query(masterModel.uid).create({ data });
    ...
    async renderChildren(idOrSlug, childUIKey, type = renderType.FLAT, menuOnly = false) {
    ...
    async renderType({ type = renderType.FLAT, criteria = {}, itemCriteria = {}, filter = null, rootPath = null }) {
      const { pluginName, service, masterModel, itemModel } = utilsFunctions.extractMeta(strapi.plugins);

      const entity = await strapi.query(masterModel.uid).findOne({
        where: {
          ...criteria,
          visible: true
        }
      ...
      if (entity && entity.id) {
        ...
        switch (type?.toLowerCase()) {
          ...
          case renderType.RFR:
            ...
            const itemParser = (item, path = "", field) => {
              ...
              const slug = isString(parentPath)
                ? slugify((first(parentPath) === "/" ? parentPath.substring(1) : parentPath).replace(/\//g, "-"))
                : undefined;
            ...
            const { items: itemsFilteredByPath, root: rootElement } = utilsFunctions.filterByPath(items, rootPath);
          ...
          default:
            const publishedItems = items.filter(utilsFunctions.filterOutUnpublished).map((item) => ({
              ...item,
    ...
    renderRFR({ items, parent = null, parentNavItem = null, contentTypes = [] }) {
      ...
      items.forEach((item) => {
        ...
        if (!parent) {
          ...
          const navLevel = navItems.filter((navItem) => navItem.type === itemType.INTERNAL.toLowerCase());
          if (!isEmpty(navLevel))
            nav = {
              ...nav,
    ...
    renderRFRPage({ item, parent }) {
      const { uiRouterKey, title, icon, path, slug, related, type, audience, menuAttached } = item;
    ...
    createBranch(items = [], masterEntity = null, parentItem = null, operations = {}) {
      ...
      return Promise.all(
        items.map(async (item) => {
          ...
          const navigationItem = await strapi.query(itemModel.uid).create({ data, populate: ["related", "items"] });
          return !isEmpty(item.items)
            ? service.createBranch(item.items, masterEntity, navigationItem, operations)
            : operations;
        })
      );
    },
      ...
      return Promise.all(
        items
          ...
          .map(async (item) => {
            ...
            await Promise.all([
              strapi.query(itemModel.uid).delete({ where: { id } }),
              this.removeRelated(related, master)
            ]);
            return !isEmpty(item.items) ? service.removeBranch(item.items, operations) : operations;
          })
      );
    },
    ...
    async updateBranch(toUpdate, masterEntity, parentItem, operations) {
      ...
      return Promise.all(
        toUpdate.map(async (item) => {
          ...
          if (updated) {
            const relatedItems = await this.getIdsRelated(related, master);
            currentItem = await databaseModel.update({
              where: { id },
              data: {
                ...params,
                related: relatedItems,
          ...
          return !isEmpty(items) ? service.analyzeBranch(items, masterEntity, currentItem, operations) : operations;
        })
      );
    },
    ...
    analyzeBranch(items = [], masterEntity = null, parentItem = null, prevOperations = {}) {
      const { service } = utilsFunctions.extractMeta(strapi.plugins);
      const { toCreate, toRemove, toUpdate } = items.reduce(
        (acc, _) => {
      ...
      return utilsFunctions
        .checkDuplicatePath(parentItem || masterEntity, toCreate.concat(toUpdate))
        .then(() =>
          Promise.all([
