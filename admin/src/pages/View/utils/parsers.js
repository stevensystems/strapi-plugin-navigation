...
export const transformItemToRESTPayload = (
  ...
  const relatedContentType = relatedType ? find(contentTypes, (ct) => ct.uid === relatedType) : undefined;
  ...
  return {
    ...
    audience: audience.map((audienceItem) => (isObject(audienceItem) ? audienceItem.value : audienceItem)),
    path: isExternal ? undefined : path,
    externalPath: isExternal ? externalPath : undefined,
    related:
      isExternal || isWrapper
        ? undefined
        : [
            {
              ...
              field:
                relatedContentType && relatedContentType.relatedField ? relatedContentType.relatedField : "navigation"
            }
          ],
...
const linkRelations = (item, config) => {
  ...
  if (shouldBuildRelated && !shouldFindRelated) {
    const relatedContentType = find(contentTypes, (ct) => ct.uid === relatedItem.__contentType, {});
...
export const transformItemToViewPayload = (payload, items = [], config) => {
  ...
  const updatedLevel = items.map((item) => {
    ...
    if (payload.viewParentId === item.viewId) {
      ...
      const updatedBranchItems = branchItems.map((iItem) => {
...
export const usedContentTypes = (items = []) =>
  items.flatMap((item) => {
